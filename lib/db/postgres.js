...
async function init(options = {}) {
  ...
  async function attachClientListeners(c) {
    ...
    c.on('error', err => {
      ...
      setConnected(false);
      // start reconnect in background (unless we're shutting down)
      if (!shuttingDown) startReconnectLoop(clientConfig).catch(() => {});
    });
    c.on('end', () => {
...
async function close() {
  try {
    // Mark shutting down so reconnect attempts stop
    shuttingDown = true;
    if (client) {
      // Remove listeners so the 'end' event doesn't trigger reconnect logic
      try { client.removeAllListeners('error'); } catch (e) { /* ignore */ }
      try { client.removeAllListeners('end'); } catch (e) { /* ignore */ }
      await client.end();
    }
  } catch (e) {
    // ignore
